...
// Use the same type as SmaajKeTaajProfile from ConfigContext
interface Profile {
  id: number;
  name: string;
  role?: string;
...
const KingIcon = ({size = 24, color = 'black'}) => (
  <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
    <Path
      d="M5 16L3 5l5.5 5L12 4l3.5 6L21 5l-2 11H5zm2.7-2h8.6l.9-5.4-2.1 1.4L12 8l-3.1 2L6.8 8.6L7.7 14z"
      fill={color}
    />
  </Svg>
);
...
const HomeScreen = () => {
  ...
  const fetchNews = async () => {
    try {
      ...
      console.error('Error fetching news for home screen:', error);
      // Silently fail for home screen - don't show error alerts
    } finally {
  ...
  const defaultNewsHeadlines = [
    {
      id: 1,
      text:
        t('Breaking: New policy announced for social welfare') ||
        'Breaking: New policy announced for social welfare',
      category: 'Politics',
    },
    {
      id: 2,
      text:
        t('Community event this weekend - register now!') ||
        'Community event this weekend - register now!',
      category: 'Events',
    },
    {
      id: 3,
      text:
        t('Education reforms to be implemented next month') ||
        'Education reforms to be implemented next month',
      category: 'Education',
    },
    {
      id: 4,
      text:
        t('Local business owner wins national award') ||
    ...
    },
    {
      id: 5,
      text:
        t('Health department issues new guidelines') ||
        'Health department issues new guidelines',
      category: 'Health',
    },
  ];

  // Create news string for marquee - use real news if available, otherwise fallback
  const getNewsHeadlines = () => {
    if (newsData.length > 0) {
      // Get top 5 news headlines
      return newsData.slice(0, 5).map(item => item.title);
    }
    return defaultNewsHeadlines.map(item => item.text);
  };
  const newsString = getNewsHeadlines().join(' • ');

  // Only show news section when there's news available
  const shouldShowNews = newsData.length > 0 || !newsLoading;
  ...
  const handleOpenURL = async (url: string | undefined) => {
    ...
    try {
      ...
      if (supported) {
        ...
        Alert.alert(
          t('Error') || 'Error',
          t('Cannot open this URL') || 'Cannot open this URL',
        );
      }
    } catch (error) {
      Alert.alert(
        t('Error') || 'Error',
        t('Failed to open URL') || 'Failed to open URL',
      );
    }
  };
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              {(selectedProfile.keyAchievements ||
                selectedProfile.communityContribution ||
                ...
                <View style={styles.modalSection}>
                  ...
                  {selectedProfile.keyAchievements &&
                    selectedProfile.keyAchievements.trim() !== '' && (
                      <View style={styles.modalTextSection}>
                        <Text style={styles.modalSubTitle}>
                          {t('Key Achievements') || 'Key Achievements'}
                        </Text>
                  ...
                  {selectedProfile.communityContribution &&
                    selectedProfile.communityContribution.trim() !== '' && (
                      <View style={styles.modalTextSection}>
                        <Text style={styles.modalSubTitle}>
                  ...
                  <View style={styles.modalSection}>
                    ...
                    <View style={styles.modalTagsContainer}>
                      {selectedProfile.hobbies.map((hobby, index) => (
                        <View
                          key={index}
                          style={[styles.modalTag, styles.modalHobbyTag]}>
              ...
              {(selectedProfile.website ||
                ...
                <View style={styles.modalSection}>
                  <Text style={styles.modalSectionTitle}>
                    {t('Links & Social Media') || 'Links & Social Media'}
                  </Text>
                  ...
                  {selectedProfile.instagram &&
                    selectedProfile.instagram.trim() !== '' && (
                      <TouchableOpacity
                        style={styles.modalLinkRow}
                        onPress={() =>
                          handleOpenURL(selectedProfile.instagram)
                        }>
                  ...
                  {selectedProfile.socialLink &&
                    selectedProfile.socialLink.trim() !== '' && (
                      <TouchableOpacity
                        style={styles.modalLinkRow}
                        onPress={() =>
                          handleOpenURL(selectedProfile.socialLink)
                        }>
                        <LinkIcon size={20} color="#7dd3c0" />
                        <Text style={styles.modalLinkText}>
                          {t('Social Link') || 'Social Link'}
                        </Text>
                        <ExternalLinkIcon size={16} color="#aaa" />
                      </TouchableOpacity>
                    )}
          ...
          {isDataStale && (
            <View style={styles.staleDataIndicator}>
              <Text style={styles.staleDataText}>
                {t('Data might be outdated. Pull to refresh.') ||
                  'Data might be outdated. Pull to refresh.'}
              </Text>
            </View>
          )}
          ...
          {loading ? (
            <View style={styles.loadingContainer}>
              ...
              <Text style={styles.loadingText}>
                {t('Loading profiles...') || 'Loading profiles...'}
              </Text>
            </View>
          ) : (
